...
public class MySQLConnection {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        // In case that item does not exit in item table, we need put item into item table
        saveItem(item);

        /*
        The reason not to use the following sql insert is user_id may be put wrongly by
        user. (ex: user_id = "1111; drop_table;" will cause entire table removed.)
        ...
        */
        ...
        // IGNORE means if (user_id, item_id) exits in the table, do nothing
        String sql = "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES (?, ?)";
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            // Note: SQL index starts from 1, not 0
    ...
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {
        ...
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, userId);
            // ResultSet can be treated like a iterator
        ...
        try {
            ...
            for (String itemId : favoriteItemIds) {
                ...
                if (rs.next()) {
                    ...
                    // Note class Item has private constructor and needs use Builder to instantiate an Item class object
